...
def get_testData(path, spike_ready=True):
    ...
    y_test = torch.FloatTensor(np.load(data_save_dir + 'labels_test.npy'))
    if spike_ready == False:
...
def get_trainValLoader(path, k=0, spike_ready=True):
...
def get_testLoader(path, spike_ready=True):
