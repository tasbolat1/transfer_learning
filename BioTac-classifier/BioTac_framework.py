...
class Dataset(data2.Dataset):
    def __init__(self, list_IDs, labels, cut_idx=None):
        # initialize
        self.labels = labels
...
class BioCNN_LSTM(nn.Module):
    ...
    def forward(self, x, h_ini, c_ini):
        ...
        # print("h_ini size", h_ini.size(), h.size())
...
def BioTac_learn(datafile, num_class, args, iteration=400):
    ...
    training_dataset = Dataset(train_ids, train_labels, cut_idx=cut_idx)
    ...
    test_dataset = Dataset(test_ids, test_labels, cut_idx=cut_idx)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # print("original data", data.shape) # (21, 1, 1, , 400, 8, 5)
        ...
        if epoch % save_interval == 0:
            torch.save(model.state_dict(), dirName + 'model_comb_loadLSTM_{}_cut_{}_epoch_'.format(load_lstm ,cut_idx) + str(epoch) +'.ckpt')
...
def main():
    ...
    BioTac_learn(datafile, num_class, args)
