...
decoded_dict = pickle.load(open(data_dir+"decoded_dict_B_0421.pkl", "rb"))
...
for class_idx in range(num_class):
    for n in range(num_samples):
        ...
        myarray = decoded_dict[class_idx][n]
        length, timestep = myarray.shape # (19, 400)
...
for col_idx in range(len(class_number_list)):
    for row_idx in range(0, num_samples):
        ...
        length, timestep = myarray.shape
        # repeat rows for better visualization
        myarray = np.repeat(myarray, repeats=10, axis=0)
        axes[row_idx, col_idx].imshow(myarray)
figname = data_dir+"figures/decoded_B_{}_class_{}_samples.png".format(len(class_number_list), num_samples)
