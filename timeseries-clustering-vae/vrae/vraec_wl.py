...
from vrae.vrae import *
from vrae.utils import *

class VRAEC_wl(BaseEstimator, nn.Module):
    ...
    :param w_r: weight for reconstruction loss
    :param w_k: weight for kl_div loss
    :param w_c: weight for classification loss
    """
    def __init__(self, num_class, sequence_length, number_of_features, hidden_size=90, hidden_layer_depth=2, latent_length=20,
                 ...
                 cuda=False, print_every=100, clip=True, max_grad_norm=5, dload='.', model_name='model.pth', header=None, w_r=1, w_k=1, w_c=1):

        super(VRAEC_wl, self).__init__()
        ...
        self.w_k = w_k
        self.w_c = w_c
        print("loss weights for vraec w_r: {}, w_k: {}, w_c: {}".format(self.w_r, self.w_k, self.w_c))
    ...
    def compute_loss(self, X, y):
        ...
        x_decoded, latent, output = self(x) # run forward()
        # compute classification acc

        # construct loss function
        cl_loss = self.cl_loss(output, y)
        loss, recon_loss, kl_loss = self._rec(x_decoded, x.detach(), self.loss_fn)
        loss += cl_loss

        return loss, recon_loss, kl_loss, cl_loss, x, latent
    ...
    def _train(self, train_loader):
        ...
        for t, (X, y) in enumerate(train_loader):
            ...
            # print("{}: check X".format(t), X.size(), y.size())
            # required to swap axes, since dataloader gives output in (batch_size x seq_len x num_of_features)
            X = X.permute(1,0,2)

            self.optimizer.zero_grad()
            loss, recon_loss, kl_loss, cl_loss, _, latent = self.compute_loss(X, y)
            # overwrite the loss with weights
            loss = self.w_r*recon_loss + self.w_k*kl_loss + self.w_c*cl_loss
            loss.backward()
        ...
        print('last batch, loss = %.4f, recon_loss = %.4f, kl_loss = %.4f, cl_loss = %.4f' % (loss.item(),
                                                                                    recon_loss.item(), kl_loss.item(), cl_loss.item()))
    ...
    def _valid(self, data_loader):
        """
        For each epoch, compute current classification accuracy
        """
        self.eval()
    ...
    def fit(self, dataset, save = False):
        ...
        for i in range(self.n_epochs):
            avg_loss = self._train(train_loader)
            acc = self._valid(train_loader)
            print('Epoch: {}, avg loss: {}, training acc: {}'.format(i, avg_loss, acc))
            epoch_train_acc.append(acc)
            self.epoch_train_acc = epoch_train_acc.copy()
    ...
    def _classify(self, z):
        """
        Perform classification on given latent representation
        """
